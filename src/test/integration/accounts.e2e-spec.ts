...
describe('Account Service', () => {
  ...
  beforeEach(async () => {
    ...
    delegationLegacyService = moduleRef.get<DelegationLegacyService>(
  ...
  describe('Accounts count', () => {
    it(`should return a number`, async () => {
      const accountsCount: number = new Number(
        await accountService.getAccountsCount(),
      );
  ...
  describe('Specific account', () => {
    describe('Account Details', () => {
      it(`should return a detailed account with account address`, async () => {
        ...
        expect(accountDetailed).toHaveStructure(
          Object.keys(new AccountDetailed()),
        );
      ...
      it(`should throw 'Account not found' error`, async () => {
        expect(
          await accountService.getAccount(accountAddress + 'a'),
        ).toBeNull();
      });
    });
    ...
    describe('Account Delegation Legacy', () => {
      it(`should return a delegation legacy for an account with address`, async () => {
        const accountDelegationLegacy =
          await delegationLegacyService.getDelegationForAddress(accountAddress);

        expect(accountDelegationLegacy).toHaveStructure(
          Object.keys(new AccountDelegationLegacy()),
        );
      });
    });
  });
