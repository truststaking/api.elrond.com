...
describe('Blocks Service', () => {
  ...
  beforeEach(async () => {
    ...
    blocks = await blocksService.getBlocks(new BlockFilter(), {
      from: 0,
      size: 25,
    });
  ...
  describe('Blocks', () => {
    ...
    it('should be sorted by timestamp in descending order', async () => {
      ...
      while (index < blocks.length) {
        ...
        expect(BigInt(blocks[index - 1].timestamp)).toBeGreaterThanOrEqual(
          BigInt(blocks[index].timestamp),
        );
        index++;
      }
    });
    ...
    it('should be filtered by shard and epoch', async () => {
      ...
      const filteredBlocks = await blocksService.getBlocks(blocksFilter, {
        from: 0,
        size: 25,
      });
