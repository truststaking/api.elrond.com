...
export class IdentitiesService {
  ...
  async getIdentities(ids: string[]): Promise<Identity[]> {
    ...
    if (ids.length > 0) {
      identities = identities.filter(
        (x) => x.identity && ids.includes(x.identity),
      );
    }
  ...
  private getStakeInfoForIdentity(
    ...
    const stake: bigint = identity.nodes
      ...
      .reduce(
        (acumulator: bigint, current: string) => acumulator + BigInt(current),
        BigInt(0),
      );
    ...
    stakeInfo.distribution = this.getStakeDistributionForIdentity(
      locked,
      identity,
    );
    ...
    stakeInfo.sort =
      stakeInfo.locked && stakeInfo.locked !== '0'
        ? parseInt(stakeInfo.locked.slice(0, -18))
        : 0;
  ...
  async getAllIdentitiesRaw(): Promise<Identity[]> {
    ...
    nodes.forEach((node) => {
      const found = identitiesDetailed.find(
        (identityDetailed) => identityDetailed.identity === node.identity,
      );
    ...
    let identities: Identity[] = identitiesDetailed.map(
      (identityDetailed: IdentityDetailed) => {
        if (identityDetailed.nodes && identityDetailed.nodes.length) {
          ...
          const stakeInfo = this.getStakeInfoForIdentity(
            identityDetailed,
            BigInt(parseInt(totalLocked)),
          );
    ...
    identities = identities.filter(
      (identity) => identity && identity.locked !== '0',
    );

    identities.sort((a, b) => {
      const aSort =
        a && a.locked && a.locked !== '0'
          ? parseInt(a.locked.slice(0, -18))
          : 0;
      const bSort =
        b && b.locked && b.locked !== '0'
          ? parseInt(b.locked.slice(0, -18))
          : 0;
      return bSort - aSort;
    });
  ...
  private processIdentityAvatar(avatar: string): string {
    return avatar.replace(
      ...
      `${this.apiConfigService.getMediaUrl()}/providers/asset`,
    );
  }
}
