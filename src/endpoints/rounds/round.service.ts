...
export class RoundService {
  ...
  private async buildElasticRoundsFilter(
    filter: RoundFilter,
  ...
  async getRoundCount(filter: RoundFilter): Promise<number> {
    ...
    elasticQueryAdapter.condition.must = await this.buildElasticRoundsFilter(
      filter,
    );
  ...
  async getRounds(filter: RoundFilter): Promise<Round[]> {
    ...
    ] = await this.buildElasticRoundsFilter(filter);
  ...
  async getRound(shard: number, round: number): Promise<RoundDetailed> {
    ...
    result.signers = result.signersIndexes.map(
      (index: number) => publicKeys[index],
    );
