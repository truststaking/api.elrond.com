...
export class ProviderService {
  ...
  async getProviders(query: ProviderFilter): Promise<Provider[]> {
    ...
    providers.forEach((element) => {
      ...
      const delegationData: DelegationData | undefined =
        providersDelegationData.find(
          (providerDelegationInfo: any) =>
            providerDelegationInfo !== null &&
            providerAddress === providerDelegationInfo.contract,
        );
      if (delegationData) {
      ...
      const nodesInfos: NodesInfos =
        this.getNodesInfosForProvider(providerNodes);
