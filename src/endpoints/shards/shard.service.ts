...
export class ShardService {
  ...
  async getAllShardsRaw(): Promise<Shard[]> {
    ...
    const validators = nodes.filter(
      ({ type, shard, status }) =>
        type === 'validator' &&
        shard !== undefined &&
        [NodeStatus.eligible, NodeStatus.waiting, NodeStatus.leaving].includes(
    ...
    return shards.map((shard) => {
      ...
      const activeShardValidators = shardValidators.filter(
        ({ online }) => online,
      );
  ...
  async setLastProcessedNonce(shardId: number, nonce: number): Promise<number> {
    return await this.cachingService.setCache<number>(
      `shardNonce:${shardId}`,
      nonce,
